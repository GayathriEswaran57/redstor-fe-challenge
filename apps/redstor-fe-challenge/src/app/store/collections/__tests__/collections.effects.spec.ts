...
describe('CollectionsEffects', () => {
  ...
  beforeEach(() => {
    unsplashServiceMock = {
      listCollections: jasmine
        .createSpy('listCollections')
        .and.returnValue(of({ response: { results: mockPhotosResponse, total: mockPhotosResponse.length } }))
    };
    TestBed.configureTestingModule({
      providers: [CollectionsEffects, provideMockActions(() => actions$), { provide: UnsplashService, useValue: unsplashServiceMock }]
    });
  ...
  describe('loadCollections$', () => {
    ...
    it('should dispatch loadCollectionsFailure when the API call fails', done => {
      ...
      unsplashServiceMock.listCollections.and.returnValue(
        of({
          response: null,
          type: 'error'
        })
